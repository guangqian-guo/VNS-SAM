...
class MaskDecoderHQ(MaskDecoder):
    def __init__(self, model_type):
        ...
        for i in range(2):
            self.hf_mlp.append(MLP(transformer_dim, transformer_dim, transformer_dim // 8, 3))
        
        self.edge_mlp = nn.ModuleList()
        for i in range(2):
            self.edge_mlp.append(MLP(transformer_dim, transformer_dim, transformer_dim // 8, 3))
        ...
        #                             )
        self.embedding_maskfeature = nn.ModuleList()
        for i in range(2):
            self.embedding_maskfeature.append(
                nn.Sequential(
        ...
        self.embedding_edgefeature = nn.ModuleList()
        for i in range(2):
            self.embedding_edgefeature.append(
                nn.Sequential(
                    nn.ConvTranspose2d(transformer_dim, transformer_dim // 4, kernel_size=2, stride=2),
                    LayerNorm2d(transformer_dim // 4),
                    nn.GELU(),
    ...
    def forward(
        ...
        for i_batch in range(batch_len):
            mask, inter_mask, iou_pred, mask_feat = self.predict_masks(
        ...
        if hq_token_only:
            return masks_hq, edge_pred, inter_masks_hq, inter_edge_pred
        else:
            return masks_sam, masks_hq, edge_pred, inter_masks_hq, inter_edge_pred, mask_feats
    ...
    def predict_masks(
        ...
        upscaled_inter_embedding_ours = self.embedding_maskfeature[0](self.output_upscaling(internal_src)) + hq_feature
        
        # vis ------------------------------
        # heatmap(upscaled_embedding_ours, None)
        #-----------------------------------
        
        upscaled_edge_embedding = self.embedding_edgefeature[-1](edge_feat)
        upscaled_inter_edge_embedding = self.embedding_edgefeature[0](edge_feat)
        ...
        edge_token = self.edge_mlp[0](mask_tokens_out[:, -1, :])
        mask_hq_token = self.hf_mlp[0](mask_tokens_out[:, -2, :] + mask_tokens_out[:, -1, :])  # b 32

        inter_edge_token = self.edge_mlp[1](inter_mask_tokens_out[:, -1, :])
        inter_mask_hq_token = self.hf_mlp[1](inter_mask_tokens_out[:, -2, :] + inter_mask_tokens_out[:, -1, :])
        ...
        return masks, inter_masks, iou_pred, upscaled_edge_embedding
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            masks_hq, edge_pred, inter_masks_hq, inter_edge_pred = net(
            ...
            loss_dict = {"loss_mask1": loss_mask1, "loss_dice1":loss_dice1, "loss_edge1": loss_edge1, "loss_mask2": loss_mask2, "loss_dice2":loss_dice2, "loss_edge2": loss_edge2}
...
def evaluate(args, net, sam, valid_dataloaders, save_preds=False, visualize=False):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            masks_sam, masks_hq, edge_pred, inter_masks_hq, inter_edge, mask_feats = net(
                image_embeddings=encoder_embedding,
                image_pe=image_pe,
                sparse_prompt_embeddings=sparse_embeddings,
                dense_prompt_embeddings=dense_embeddings,
            ...
            masks_hq = masks_hq + masks_sam + inter_masks_hq   # mask coupled 
