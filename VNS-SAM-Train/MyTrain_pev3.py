...
# -----------------------------------------------
# add noise box prompt, noise level is random selected, from 0 to 0.4.
# 2024.2.25
# -----------------------------------------------
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, val_set_name, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            # input prompt
            # input_keys = ['box', 'noise_box', '1point', '3point', '5point', '10point', 'noise_mask']
            input_keys = ['box', 'noise_box', 'point', 'noise_mask']

            labels_box = misc.masks_to_boxes(labels[:,0,:,:])
            scale_select_list = [0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4]
            labels_noise_box = misc.box_noise(labels_box, box_noise_scale=random.choice(scale_select_list))
            
            
            try:
                num_points = [1,3,5,10]
                
                # labels_points_1 = misc.masks_sample_points(labels[:,0,:,:], k=1)
                # labels_points_3 = misc.masks_sample_points(labels[:,0,:,:], k=3)
                # labels_points_5 = misc.masks_sample_points(labels[:,0,:,:], k=5)
                labels_points_10 = misc.masks_sample_points(labels[:,0,:,:], k=10)
            except:
                # less than 10 points
                input_keys = ['box', 'noise_box', 'noise_mask']
            ...
            for b_i in range(len(imgs)):
                ...
                if input_type == 'box':
                    ...
                    point_coords = labels_points_10[b_i:b_i+1]
                    dict_input['point_coords'] = point_coords
                    dict_input['point_labels'] = torch.ones(point_coords.shape[1], device=point_coords.device)[None,:]
                # elif input_type == '5point':
                #     point_coords = labels_points_5[b_i:b_i+1]
                #     dict_input['point_coords'] = point_coords
                #     dict_input['point_labels'] = torch.ones(point_coords.shape[1], device=point_coords.device)[None,:]
                # elif input_type == '3point':
                #     point_coords = labels_points_3[b_i:b_i+1]
                #     dict_input['point_coords'] = point_coords
                #     dict_input['point_labels'] = torch.ones(point_coords.shape[1], device=point_coords.device)[None,:]
                # elif input_type == '1point':
                #     point_coords = labels_points_1[b_i:b_i+1]     
                #     dict_input['point_coords'] = point_coords
                #     dict_input['point_labels'] = torch.ones(point_coords.shape[1], device=point_coords.device)[None,:]
            ...
            loss =  loss1 + loss2 + loss_masks_dense
            ...
            loss_dict = {"loss_mask1": loss_mask1, "loss_dice1":loss_dice1, "loss_edge1": loss_edge1, 
                         ...
                         "loss_masks_dense": loss_masks_dense}
...
def evaluate(args, net, sam, valid_dataloaders, val_set_name, save_preds=False, visualize=False):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            labels_noise_box = misc.box_noise(labels_box, box_noise_scale=0.2)
            
            labels_points = misc.masks_sample_points(labels_val[:,0,:,:], k=5)
            ...
            masks_sam, masks_hq, edge_pred, inter_masks_hq, inter_edge, masks_dense, mask_feats, attn_maps = net(
                image_embeddings=encoder_embedding,
                image_pe=image_pe,
                sparse_prompt_embeddings=sparse_embeddings,
                dense_prompt_embeddings=dense_embeddings,
            ...
            # masks_hq = masks_hq + masks_sam + masks_dense   # mask coupled 
