...
class SAMAggregatorNeck(nn.Module):
    ...
    def forward(self, image_embedding, intern_embeddings):
        inner_states = intern_embeddings
        ...
        img_feat = image_embedding + x

        # img_feat = self.up_layers[0](x) + x
        # img_feat = self.up_layers[1](img_feat)
        # img_feat = self.up_sample_layers[0](x)
...
class MaskDecoderHQ(MaskDecoder):
    def __init__(self, model_type):
        ...
        # print(self.mask_tokens)
        # print(self.output_hypernetworks_mlps)
        self.neck = SAMAggregatorNeck()
        
        for name, para in self.named_parameters():
            # if para.requires_grad == True:
            #     print(name)
            if "output_upscaling" in name or "masks_tokens" in name or "output_hypernetworks_mlps" in name:
    ...
    def forward(
        ...
        image_embeddings = self.neck(image_embeddings, interm_embeddings)
        ...
        masks = masks[:, mask_slice, :, :]  # 4 1 256 256, bs num, w, h
    ...
    def predict_masks(
        ...
        # upscaled_embedding_ours = self.embedding_maskfeature(upscaled_embedding_sam) + hq_feature  # 和交互后的image embedding 又做了一个融合
        ...
        # masks_ours = (hyper_in[:,4:] @ upscaled_embedding_ours.view(b, c, h * w)).view(b, -1, h, w)  # 和mask token做点乘，作为输出的mask。
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            with torch.no_grad():
                batched_output, interm_embeddings = sam(batched_input, multimask_output=False)  # intern feat: 4 64 64 1024

            batch_len = len(batched_output)
            encoder_embedding = torch.cat([batched_output[i_l]['encoder_embedding'] for i_l in range(batch_len)], dim=0)  # 4,256 64 ,64
