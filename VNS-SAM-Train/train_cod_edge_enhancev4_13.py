...
class MaskDecoderHQ(MaskDecoder):
    ...
    def predict_masks(
        ...
        tokens = torch.cat((output_tokens, sparse_prompt_embeddings), dim=1)
        # Expand per-image data in batch direction to be per-mask
        image_embeddings_hq = image_embeddings + edge_feat
        src_hq = torch.repeat_interleave(image_embeddings_hq, tokens.shape[0], dim=0) 
        src_hq = src_hq + dense_prompt_embeddings

        
        # Run the transformer
        hs, src_hq = self.transformer(src_hq, pos_src, tokens)
        mask_tokens_out_hq = hs[:, self.num_mask_tokens : (2 + self.num_mask_tokens), :]


        # Upscale mask embeddings and predict masks using the mask tokens
        src = src.transpose(1, 2).view(b, c, h, w)
        src_hq = src_hq.transpose(1, 2).view(b, c, h, w)
        
        upscaled_embedding_sam = self.output_upscaling(src)
        # print(self.embedding_maskfeature(upscaled_embedding_sam).shape)   # 1 32 256 256 
        upscaled_embedding_ours = (self.embedding_maskfeature(self.output_upscaling(src_hq)) + hq_feature) # b 32 256 256
        ...
        edge_token = self.edge_mlp(mask_tokens_out_hq[:, -1, :])
