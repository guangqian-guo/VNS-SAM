...
class MaskDecoderHQ(MaskDecoder):
    ...
    def forward(
        ...
        edge_dense = dense_preds[:, slice(1,2), :, :]


        if hq_token_only:
            return masks_hq, edge_pred, masks_dense, edge_dense
        else:
            return masks_sam, masks_hq, edge_pred, masks_dense, mask_feats
    ...
    def predict_masks(
        ...
        dense_edge = self.edge_attn(feat, ed_token, ed_token) + feat  # 1 4096 256
        ...
        dense_edge = dense_edge.transpose(1, 2).view(b, 1, h, w) 

        dense_ = torch.cat([dense_pred, dense_edge], dim=1)
...
def get_args_parser():
    ...
    parser.add_argument('--resume-from', type=str, default=None, help='load checkpoint')
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            masks_hq,  edge_pred, masks_dense, edge_dense = net(
            ...
            loss =  (loss_mask + loss_dice) + loss_edge + loss_masks_dense + loss_edge_dense
            
            loss_dict = {"loss_mask": loss_mask, "loss_dice":loss_dice, "loss_edge": loss_edge, "loss_mask_dense": loss_masks_dense, "loss_edge_dense": loss_edge_dense}
...
def evaluate(args, net, sam, valid_dataloaders, save_preds=False, visualize=False):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            masks_sam, masks_hq, edge_pred, masks_dense, mask_feats = net(
                image_embeddings=encoder_embedding,
                image_pe=image_pe,
                sparse_prompt_embeddings=sparse_embeddings,
                dense_prompt_embeddings=dense_embeddings,
