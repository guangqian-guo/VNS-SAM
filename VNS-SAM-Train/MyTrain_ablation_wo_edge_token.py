...
class MaskDecoderEnhance(MaskDecoder):
    def __init__(self, model_type):
        ...
        self.hf_token = nn.Embedding(1, transformer_dim)  # 一个mask token， 一个edge token
        
        self.hf_mlp = (MLP(transformer_dim, transformer_dim, transformer_dim // 8, 3))
        
        # self.edge_mlp = (MLP(transformer_dim, transformer_dim, transformer_dim // 8, 3))

        self.num_mask_tokens = self.num_mask_tokens + 1   #NOTE:  之前都设置错了，都是 +1，导致mask token好像根本没有学到。--2023.12.11
    ...
    def forward(
        ...
        hq_features = self.neck(interm_embeddings)    # 1 32 256 256, 1 1 256 256
        ...
        for i_batch in range(batch_len):
            mask, inter_mask, dense_pred, iou_pred, attn_map = self.predict_masks(
        ...
        if hq_token_only:
            return masks_hq, inter_masks_hq, masks_dense
        else:
            return masks_sam, masks_hq, inter_masks_hq, masks_dense, mask_feats, attn_maps
    ...
    def predict_masks(
        ...
        inter_masks  = torch.cat([inter_masks_ours], dim=1)

        iou_pred = self.iou_prediction_head(iou_token_out)
        
        return masks, inter_masks, dense_, iou_pred, attn_map
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, val_set_name, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            masks_hq, inter_masks_hq, masks_dense = net(
            ...
            loss_dict = {"loss_mask1": loss_mask1, "loss_dice1":loss_dice1, 
                         "loss_mask2": loss_mask2, "loss_dice2":loss_dice2,
...
def evaluate(args, net, sam, valid_dataloaders, val_set_name, save_preds=False, visualize=False):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            masks_sam, masks_hq, inter_masks_hq, masks_dense, mask_feats, attn_maps = net(
                image_embeddings=encoder_embedding,
                image_pe=image_pe,
                sparse_prompt_embeddings=sparse_embeddings,
                dense_prompt_embeddings=dense_embeddings,
