# edge token 和 mask token都加上高频信息。
...
class SAMAggregatorNeck(nn.Module):
    def __init__(
        ...
        # high feat
        self.arb = HFA(in_channels[0], lf=False)
    ...
    def forward(self, inputs):
        ...
        # wf1 = self.T(inner_states[0])

        inner_states = [einops.rearrange(inner_states[idx], 'b h w c -> b c h w') for idx in range(len(self.in_channels))]
        wf1  = self.T(inner_states[0])
        wf2 = self.T(inner_states[1])
        wf3 = self.T(inner_states[2])
        wf4 = self.T(inner_states[3])

        HH = wf1[3] + wf2[3] + wf3[3] + wf4[3]
        # LL = wf1[0] + wf2[0] + wf3[0] + wf4[0]

        HH = self.arb(HH)
        high_freq_feats = self.dePixelShuffle(HH)
                
        high_freq_tokens = self.token_conv(HH).squeeze(2).squeeze(2)
        ...
        return img_feat, edge_feat, high_freq_tokens, high_freq_feats
...
class MaskDecoderHQ(MaskDecoder):
    ...
    def forward(
        ...
        hq_features, edge_pred, high_freq_tokens, high_freq_feats = self.neck(interm_embeddings)    # 1 32 256 256, 1 1 256 256, 1,256, 1, 256 64 64
        ...
        for i_batch in range(batch_len):
            mask, iou_pred, mask_feat = self.predict_masks(
                ...
                high_freq_token = high_freq_tokens[i_batch],
                high_freq_feat = high_freq_feats[i_batch].unsqueeze(0)
            )
            masks.append(mask)
    ...
    def predict_masks(
        ...
        high_freq_token: torch.Tensor,
        high_freq_feat: torch.Tensor,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Predicts masks. See 'forward' for more details."""


        # self.hf_token = self.hf_token +  high_freq_token 
        hf_token_weight = self.hf_token.weight
        hf_token_weight  = hf_token_weight + high_freq_token.unsqueeze(0)

        # hf_token_weight = torch.cat((hf_token_weight[0].unsqueeze(0) + high_freq_token.unsqueeze(0), hf_token_weight[1].unsqueeze(0)), dim=0)
        
        output_tokens = torch.cat([self.iou_token.weight, self.mask_tokens.weight, hf_token_weight], dim=0)
        output_tokens = output_tokens.unsqueeze(0).expand(sparse_prompt_embeddings.size(0), -1, -1)
        tokens = torch.cat((output_tokens, sparse_prompt_embeddings), dim=1)
        
        # Expand per-image data in batch direction to be per-mask
        image_embeddings = image_embeddings + 0.3 * edge_feat + high_freq_feat  # 
        ...
        return masks, iou_pred, upscaled_embedding_ours
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            loss = (1-args.weight) * (loss_mask + loss_dice) + args.weight * loss_edge
