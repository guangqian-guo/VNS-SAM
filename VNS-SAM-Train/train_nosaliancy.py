...
class MaskDecoderHQ(MaskDecoder):
    ...
    def predict_masks(
        ...
        output_tokens = torch.cat([self.iou_token.weight, self.mask_tokens.weight, self.hf_token.weight], dim=0)  # 添加了一个hf token
        ...
        src = src.transpose(1, 2).view(b, c, h, w)  # 经过交叉注意力交互后的 embedding 
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    logging.info('{} Start Training!'.format(datetime.now()))
    ...
    for epoch in range(epoch_start,epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            # print(metric_logger.meters["loss_mask"], 
            #       metric_logger.meters["loss_mask"].global_avg, 
            #       metric_logger.meters["loss_mask"].avg,
            #       metric_logger.meters["loss_mask"].median,
            #       metric_logger.meters["loss_mask"].max)
...
def evaluate(args, net, sam, valid_dataloaders, visualize=False):
    ...
    if dist.get_rank() == 0:
        logging.info('test stats: {}'.format(resstat))
