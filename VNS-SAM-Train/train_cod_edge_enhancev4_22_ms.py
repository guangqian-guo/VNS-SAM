...
class MaskDecoderHQ(MaskDecoder):
    def __init__(self, model_type):
        ...
        self.edge_attn = Attention(transformer_dim, 8, 2)
        self.dense_pred_head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
            nn.Conv2d(transformer_dim, 1, 3, 1, 1),
        )
        self.dense_edge_head = nn.Sequential(
            nn.Conv2d(transformer_dim, transformer_dim, 3, 1, 1),
            nn.BatchNorm2d(transformer_dim),
            nn.ReLU(),
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
    ...
    def forward(
        ...
        for i_batch in range(batch_len):
            mask, dense_pred, iou_pred, mask_feat = self.predict_masks(
        ...
        inter_masks_dense = dense_preds[:, slice(1,2), :, :]
        edge_dense = dense_preds[:, slice(2,3), :, :]
        inter_edge_dense = dense_preds[:, slice(3,4), :, :]

        if hq_token_only:
            return masks_hq, edge_pred, masks_dense, inter_masks_dense, edge_dense, inter_edge_dense
        else:
            return masks_sam, masks_hq, edge_pred, masks_dense, inter_masks_dense, mask_feats
    ...
    def predict_masks(
        ...
        image_embeddings = image_embeddings + 0.5 * edge_feat
        ...
        # Run the transformer
        hs_list, src_list = self.transformer(src, pos_src, tokens)

        inter_hs = hs_list[: -1][0]
        inter_src = src_list[: -1][0]
        hs = hs_list[-1]
        src = src_list[-1]
        ...
        inter_mask_tokens_out = inter_hs[:, 1 : self.num_mask_tokens, :]

        # dense pred last layer
        hq_token = mask_tokens_out[:, -2, :].unsqueeze(1)
        ed_token = mask_tokens_out[:, -1, :].unsqueeze(1)
        feat = src

        dense_pred = self.dense_attn(feat, hq_token, hq_token)  # 1 4096 256
        dense_edge = self.edge_attn(feat, ed_token, ed_token)   # 1 4096 256

        dense_pred = dense_pred.transpose(1, 2).view(b, c, h, w)
        dense_edge = dense_edge.transpose(1, 2).view(b, c, h, w) 
        dense_pred = self.dense_pred_head(dense_pred)
        dense_edge = self.dense_edge_head(dense_edge)

        # dense  pred inter layer
        inter_hq_token = inter_mask_tokens_out[:, -2, :].unsqueeze(1)
        inter_ed_token = inter_mask_tokens_out[:, -1, :].unsqueeze(1)
        inter_feat = inter_src

        inter_dense_pred = self.dense_attn(inter_feat, inter_hq_token, inter_hq_token)  # 1 4096 256
        inter_dense_edge = self.edge_attn(inter_feat, inter_ed_token, inter_ed_token)   # 1 4096 256

        inter_dense_pred = inter_dense_pred.transpose(1, 2).view(b, c, h, w)
        inter_dense_edge = inter_dense_edge.transpose(1, 2).view(b, c, h, w) 
        inter_dense_pred = self.dense_pred_head(inter_dense_pred)
        inter_dense_edge = self.dense_edge_head(inter_dense_edge)

        dense_ = torch.cat([dense_pred, inter_dense_pred, dense_edge, inter_dense_edge], dim=1)
        ...
        upscaled_embedding_ours = (self.embedding_maskfeature(upscaled_embedding_sam) + hq_feature) # b 32 256 256
        ...
        mask_hq_token = self.hf_mlp(mask_tokens_out[:, -2, :] + mask_tokens_out[:, -1, :])  # b 32
        ...
        return masks, dense_, iou_pred, upscaled_edge_embedding
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            masks_hq,  edge_pred, masks_dense, inter_masks_dense, edge_dense, inter_edge_dense = net(
            ...
            loss_masks_dense = structure_loss(masks_dense, labels/255.0)
            loss_masks_dense_inter = structure_loss(inter_masks_dense, labels/255.0)

            loss_edge = dice_loss_edge(edge_pred, edges)   # TODO !!!
            loss_edge_dense = dice_loss_edge(edge_dense, edges)
            loss_edge_dense_inter = dice_loss_edge(inter_edge_dense, edges)

            loss =  (loss_mask + loss_dice) + loss_edge + loss_masks_dense + loss_edge_dense + loss_masks_dense_inter + loss_edge_dense_inter
            
            loss_dict = {"loss_mask": loss_mask, "loss_dice":loss_dice, "loss_edge": loss_edge, "loss_mask_dense": loss_masks_dense, "loss_edge_dense": loss_edge_dense, "loss_masks_dense_inter": loss_masks_dense_inter, "loss_edge_dense_inter": loss_edge_dense_inter}
...
def evaluate(args, net, sam, valid_dataloaders, save_preds=False, visualize=False):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            masks_sam, masks_hq, edge_pred, masks_dense, inter_masks_dense, mask_feats = net(
                image_embeddings=encoder_embedding,
                image_pe=image_pe,
                sparse_prompt_embeddings=sparse_embeddings,
                dense_prompt_embeddings=dense_embeddings,
            ...
            masks_hq = masks_hq + masks_sam + masks_dense + inter_masks_dense# mask coupled 
            ...
            # vis heatmap
            # heatmap(mask_feats, inputs_val.squeeze(0), os.path.join(save_dir, img_name))
            # continue
            # save predictions
            if save_preds:
                os.makedirs(args.output, exist_ok=True)
...
if __name__ == "__main__":
    ...
    dataset_cod_val = {"name": "cod",
                       "im_dir": "/mnt/nvme1n1/Guo/Project/sam-hq-main/train/data/COD/TrainDataset/high-freq-imgs",
                    "gt_dir": "./data/COD/TrainDataset/GT",
                    "im_ext": ".jpg",
                    "gt_ext": ".png"}
