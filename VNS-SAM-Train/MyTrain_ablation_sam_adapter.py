...
from segment_anything_training.modeling import mask_decoder
matplotlib.use('Agg')
...
class sam_adapter(nn.Module):
    ...
    def __init__(self, args):
        ...
        # self.sam.train(
        self.mask_decoder = MaskDecoderTT(args.model_type)
    ...
    def init_weight(self, checkpoint):
        ...
        for name, para in self.named_parameters():
            if "prompt_encoder" in name or ("image_encoder" in name and "prompt_generator" not in name) or "mask_decoder" in name:
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, val_set_name, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            masks_retrain = net(batched_input, multimask_output=False)
