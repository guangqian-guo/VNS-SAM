...
class MaskDecoderHQ(MaskDecoder):
    ...
    def forward(
        ...
        masks_pred = self.mask_pred_head(image_embeddings)
        ...
        if hq_token_only:
            return masks_hq, edge_pred, masks_pred
        else:
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            masks_hq, edge_pred, masks_pred = net(
            ...
            loss_mask_aux = structure_loss(masks_pred, labels/255.0)
            
            loss = (loss_mask + loss_dice) + loss_edge + loss_mask_aux
            
            loss_dict = {"loss_mask": loss_mask, "loss_dice":loss_dice, "loss_edge": loss_edge, "loss_mask_aux": loss_mask_aux}
