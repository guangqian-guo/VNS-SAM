...
class SAMAggregatorNeck(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # edge pred
        edge_feat = self.edge_pred_layer0(x)
        edge_feat = self.edge_pred_head(edge_feat)
        edge_pred = torch.sigmoid(F.interpolate(edge_feat, scale_factor=4, mode='bilinear', align_corners=False))

        # high quality features
        img_feat = self.hqfeat_layer0(x)
        img_feat = self.hqfeat_head(img_feat)
        img_feat = self.up_sample_layers[0](img_feat)

        return img_feat, edge_pred
...
class MaskDecoderHQ(MaskDecoder):
    ...
    def forward(
        ...
        for i_batch in range(batch_len):
            mask, iou_pred = self.predict_masks(
                ...
                edge_pred = edge_pred[i_batch].unsqueeze(0)
            )
            masks.append(mask)
    ...
    def predict_masks(
        ...
        upscaled_embedding_ours = (self.embedding_maskfeature(upscaled_embedding_sam) + hq_feature) * (edge_pred +1)  # 边缘
