...
class MaskDecoderHQ(MaskDecoder):
    ...
    def predict_masks(
        ...
        # Run the transformer
        hs, src = self.transformer(src, pos_src, tokens)   # hs: 1 7 256

        iou_token_out = hs[:, 0, :]  
        mask_tokens_out = hs[:, 1 : (2+self.num_mask_tokens), :]   #NOTE： 这里也设置的不对，应该就是self.num_mask_tokens，之前是2+self.num_mask_tokens  2023.12.11
