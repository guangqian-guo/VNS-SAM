...
def evaluate(args, net, sam, valid_dataloaders, save_preds=False, visualize=False):
    ...
    print("Validating...")
    test_stats = {}
    dataset_name = ['CVC-ColonDB','CVC-300', 'CVC-ClinicDB', 'Kvasir']
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            masks_hq = masks_hq + masks_sam  # coupled
            
            # iou = compute_iou(masks_hq, labels_ori)
            # boundary_iou = compute_boundary_iou(masks_hq, labels_ori)

            # binary_mask = F.interpolate(masks_hq, size=labels_val.shape[2:], mode='bilinear', align_corners=False)
            # binary_mask = torch.sigmoid(binary_mask.detach().cpu())
            # binary_mask =  (binary_mask>0.5).float()
            # binary_gt_mask = labels_val.detach().cpu()
            ...
            # dice(binary_mask.squeeze(0), binary_gt_mask.squeeze(0))
            # gd(binary_mask.squeeze(0), binary_gt_mask.squeeze(0))
            # miou(binary_mask.squeeze(0), binary_gt_mask.squeeze(0))
            
            # final_dice = dice.aggregate().numpy()[0]
            # final_gd = gd.aggregate().numpy()[0]
            # final_iou = miou.aggregate().numpy()[0]
...
if __name__ == "__main__":
    ...
    train_datasets=[dataset_poly]
    valid_datasets = [dataset_colondb_val, dataset_cvc300_val, dataset_clinicdb_val, dataset_kvasir_val]
