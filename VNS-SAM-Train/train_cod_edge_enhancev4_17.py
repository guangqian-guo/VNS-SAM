...
class MaskDecoderHQ(MaskDecoder):
    def __init__(self, model_type):
        ...
        #                             )
        self.fusion_factor_conv = nn.Sequential(
    ...
    def forward(
        ...
        for i_batch in range(batch_len):
            mask, iou_pred, mask_feat, fusion_factor = self.predict_masks(
        ...
        fusion_factors = torch.cat(fusion_factors, 0)
        ...
        b, _, _, _ = masks_hq.shape
        masks_hq = masks_hq * fusion_factors.view(b,1,1,1) + masks_sam * (1 - fusion_factors.view(b,1,1,1))
    ...
    def predict_masks(
        ...
        image_embeddings = image_embeddings + 0.1 * edge_feat
        # compute fusion factor
        fusion_factor = self.fusion_factor_conv(image_embeddings).squeeze(0).squeeze(0)
        fusion_factor = torch.sigmoid(fusion_factor)
        print(fusion_factor)
        ...
        return masks, iou_pred, upscaled_edge_embedding, fusion_factor
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            low_res_masks = [batched_output[i_l]['low_res_logits'] for i_l in range(batch_len)]
