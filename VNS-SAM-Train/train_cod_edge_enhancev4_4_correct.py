...
class MaskDecoderHQ(MaskDecoder):
    ...
    def forward(
        ...
        edge_pred = masks[:,slice(self.num_mask_tokens-1, self.num_mask_tokens), :, :]
    ...
    def predict_masks(
        ...
        edge_pred = torch.sigmoid((edge_token @ upscaled_edge_embedding.view(b, c, h * w)).view(b, -1, h, w))
        
        upscaled_embedding_ours = (self.embedding_maskfeature(upscaled_embedding_sam) + hq_feature)* (edge_pred+1)
