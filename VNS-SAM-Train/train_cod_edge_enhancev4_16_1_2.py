...
class SAMAggregatorNeck(nn.Module):
    ...
    def forward(self, inputs):
        ...
        HH = self.arb(HH)
        high_freq_feats = self.dePixelShuffle(HH)  # b 32  64 64 
...
class MaskDecoderHQ(MaskDecoder):
    ...
    def predict_masks(
        ...
        hf_token_weight = torch.cat((hf_token_weight[0].unsqueeze(0) + 0.5 * high_freq_token.unsqueeze(0), hf_token_weight[1].unsqueeze(0)), dim=0)
        ...
        image_embeddings = image_embeddings + 0.3 * edge_feat + 0.5 * high_freq_feat  # 
