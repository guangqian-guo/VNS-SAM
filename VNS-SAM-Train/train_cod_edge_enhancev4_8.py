...
class SAMAggregatorNeck(nn.Module):
    def __init__(
        ...
        # texture layer
        self.texture_pred_layer0 = nn.Sequential(
                BasicConv2d(
                    inner_channels,
                    inner_channels,
                    kernel_size=3,
        ...
        self.texture_pred_layer1 = nn.Sequential(
            BasicConv2d(
                    inner_channels,
                    inner_channels,
                    kernel_size=1
        ...
        self.texture_pred_head = nn.Sequential(
            BasicConv2d(
                    inner_channels,
                    inner_channels,
                    kernel_size=1
    ...
    def forward(self, inputs):
        ...
        # texture pred
        tex_feat = self.texture_pred_layer0(x) + x
        tex_feat = self.texture_pred_layer1(tex_feat)
        tex_feat = self.texture_pred_head(tex_feat)
        ...
        return img_feat, edge_feat, tex_feat
...
class MaskDecoderHQ(MaskDecoder):
    def __init__(self, model_type):
        ...
        self.tex_pred_head = nn.Sequential(
            nn.ConvTranspose2d(transformer_dim, transformer_dim // 4, kernel_size=2, stride=2),
            LayerNorm2d(transformer_dim // 4),
            nn.GELU(),
            ...
            LayerNorm2d(transformer_dim // 4),
            nn.GELU(),
            nn.Conv2d(transformer_dim // 4, 1, 3, 1, 1)
        )
    ...
    def forward(
        ...
        hq_features, edge_pred, tex_feature = self.neck(interm_embeddings)    # 1 32 256 256, 1 1 256 256
        tex_pred = self.tex_pred_head(tex_feature)
        ...
        for i_batch in range(batch_len):
            mask, iou_pred, mask_feat = self.predict_masks(
                ...
                tex_feat = tex_feature[i_batch].unsqueeze(0)
            )
            masks.append(mask)
        ...
        if hq_token_only:
            return masks_hq, edge_pred, tex_pred
        else:
    ...
    def predict_masks(
        ...
        image_embeddings = image_embeddings + edge_feat + tex_feat
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            masks_hq, edge_pred, tex_pred = net(
            ...
            loss_tex = structure_loss(tex_pred, labels/255.0)
            
            loss = loss_mask + loss_dice + loss_edge + loss_tex
            
            loss_dict = {"loss_mask": loss_mask, "loss_dice":loss_dice, "loss_edge": loss_edge, "loss_tex": loss_tex}
