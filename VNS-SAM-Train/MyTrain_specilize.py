...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, val_set_name, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            masks_sam, masks_hq, edge_pred, inter_masks_hq, inter_edge_pred, masks_dense = net(
            ...
            loss_mask_sam, loss_dice_sam = loss_masks(masks_sam, labels/255.0, len(masks_sam))
            loss_sam = loss_mask_sam + loss_dice_sam
            
            loss =  loss1 + loss2 + loss_masks_dense + loss_sam 
            ...
            loss_dict = {"loss_mask1": loss_mask1, "loss_dice1":loss_dice1, "loss_edge1": loss_edge1, 
                         ...
                         "loss_masks_dense": loss_masks_dense, "loss_sam": loss_sam}
    ...
    test_stats = evaluate(args, net, sam, valid_dataloaders, val_set_name, save_preds=True)
...
def evaluate(args, net, sam, valid_dataloaders, val_set_name, save_preds=False, visualize=False):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            # res = res * 255
            # mask = labels_ori.data.cpu().numpy().squeeze().squeeze()

            # mask = cv2.imread(ori_gt_path[0], cv2.IMREAD_GRAYSCALE)
