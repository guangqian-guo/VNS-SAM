...
from segment_anything_training.modeling import ImageEncoderViTAdapter, PromptEncoder
...
def build_encoder(encoder_embed_dim, encoder_depth, encoder_num_heads, encoder_global_attn_indexes):
    prompt_embed_dim = 256
    image_size = 1024
    vit_patch_size = 16
    image_embedding_size = image_size // vit_patch_size
    
    return ImageEncoderViTAdapter(
            depth=encoder_depth,
...
class sam_adapter(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.image_encoder = encoder_model_registry[args.model_type]()
        ...
        # self.sam.train(
        self.mask_decoder = MaskDecoderEnhance(args.model_type)
        ...
        self.init_weight(args.checkpoint)
    ...
    def init_weight(self, checkpoint):
        if checkpoint is not None:
            ...
            self.load_state_dict(state_dict, strict=False)    # NOTE: strict=False
        
        
        for name, para in self.named_parameters():
            if "prompt_encoder" in name or ("image_encoder" in name and "prompt_generator" not in name):
                para.requires_grad = False
            else:
                if "mask_decoder" not in name:
                    para.requires_grad = True
                
                elif "internal" in name or "hf" in name or "edge" in name or "neck" in name or "feature" in name or "dense" in name:
                    para.requires_grad = True
    ...
    def forward(self, 
        ...
        # batched_output, inter_emb = self.sam(batched_input, multimask_output)
        ...
        for image_record, curr_embedding in zip(batched_input, image_embeddings):
            ...
            # elif "noise_boxes" in image_record:
            #     boxes = image_record["noise_boxes"]
            # else:
            #     boxes = None
        ...
        masks_hq, edge_pred, inter_masks_hq, inter_edge_pred, masks_dense = self.mask_decoder(
        ...
        return masks_hq, edge_pred, inter_masks_hq, inter_edge_pred, masks_dense
...
def main(net, train_datasets, valid_datasets, args):
    ...
    if not args.eval:
        ...
        evaluate(args, net, valid_dataloaders, val_set_name, False, args.visualize)
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, val_set_name, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            masks_hq, edge_pred, inter_masks_hq, inter_edge_pred, masks_dense = net(
            ...
            loss1 = loss_mask1 + loss_dice1 + loss_edge1
            ...
            loss2 = loss_mask2 + loss_dice2 + loss_edge2
            
            # loss_mask_dense, loss_dice_dense = loss_masks(masks_dense, labels/255.0, len(masks_dense))
            loss_dense = structure_loss(masks_dense, labels/255.0)
            # loss_dense = loss_mask_dense + loss_dice_dense
            
            loss =  loss1 + loss2 + loss_dense

            # loss_dict = {"loss_mask": loss_mask, "loss_dice":loss_dice, "loss_edge": loss_edge}
            loss_dict = {"loss_mask1": loss_mask1, "loss_dice1":loss_dice1, "loss_edge1": loss_edge1, 
                         "loss_mask2": loss_mask2, "loss_dice2":loss_dice2, "loss_edge2": loss_edge2,
                         "loss_masks_dense": loss_dense}
    ...
    test_stats = evaluate(args, net, valid_dataloaders, val_set_name, save_preds=False)
...
def evaluate(args, net, valid_dataloaders, val_set_name, save_preds=False, visualize=False):
    net.eval()
    print("Validating...")
    if dist.get_rank() == 0:
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            with torch.no_grad():
                masks_hq, edge_pred, inter_masks_hq, inter_edge, masks_dense = net(
                    batched_input, multimask_output=False
                )
                    
            if args.results_fusion:
                masks_dense = F.interpolate(masks_dense, size = masks_hq.shape[2:], mode='bilinear', align_corners=False)
                masks_hq = masks_hq + masks_dense # mask coupled 
            else:
                masks_hq = masks_hq
