...
class MaskDecoderHQ(MaskDecoder):
    ...
    def forward(
        ...
        for i_batch in range(batch_len):
            mask, mask_ours, iou_pred, mask_feat = self.predict_masks(
            ...
            masks.append(mask)
            masks_ours.append(mask_ours)
            iou_preds.append(iou_pred)
            mask_feats.append(mask_feat)
            
        masks = torch.cat(masks,0)
        masks_ours = torch.cat(masks_ours)
        ...
        masks_hq = masks_ours
        edge_pred = torch.sigmoid(masks[:,slice(self.num_mask_tokens-2, self.num_mask_tokens-1), :, :])
    ...
    def predict_masks(
        ...
        upscaled_embedding_ours = (self.embedding_maskfeature(upscaled_embedding_sam) + hq_feature) # b 32 512 512
        ...
        masks = torch.cat([masks_sam, edge_pred],dim=1)

        iou_pred = self.iou_prediction_head(iou_token_out)
        
        return masks, masks_ours, iou_pred, upscaled_edge_embedding
...
def evaluate(args, net, sam, valid_dataloaders, save_preds=False, visualize=False):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            if masks_hq.shape == masks_sam.shape:
                masks_hq = masks_hq + masks_sam  # mask coupled 
            else:
                masks_sam = F.interpolate(masks_sam, size=masks_hq.shape[2:], mode='bilinear',align_corners=False)
