...
# add multi_stage,        2023.12.19 多尺度预测        --ggq
# add multi_stage_dense,  2023.12.21 多尺度的密集预测   --ggq  
# ----------------------------------------------------------- #
...
class TwoWayTransformer(nn.Module):
    def __init__(
        ...
        multi_stage_dense: bool = False, 
        return_attnmap: bool = False  # return attntion map for vis
    ) -> None:
        """
        ...
        self.multi_stage_dense = multi_stage_dense
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            queries, keys, attn_map = layer(  # self attn, token2img img2token
                queries=queries,
                keys=keys,
            ...
            if self.multi_stage and i < self.depth-1:  # 在每个中间层添加一个额外额输出。
                q = queries + point_embedding
                k = keys + image_pe
                attn_out, _ = self.internal_attn_token_to_image(q=q, k=k, v=keys)  # _ is attn map
                queries_ = queries + attn_out
                queries_ = self.norm_internal_attn(queries_)
                multi_queries.append(queries_)
                multi_keys.append(keys)
                
            
            if self.multi_stage_dense and i < self.depth - 1:
                multi_queries_dense.append(queries)
                multi_keys_dense.append(keys)
        ...
        if self.multi_stage:
            return multi_queries, multi_keys, attn_map
        elif self.multi_stage_dense:
            return multi_queries_dense, multi_keys_dense
        elif self.return_attnmap:
