...
class MaskDecoderHQ(MaskDecoder):
    ...
    def forward(
        ...
        for i_batch in range(batch_len):
            mask, inter_mask, dense_pred, iou_pred, mask_feat = self.predict_masks(
        ...
        if hq_token_only:
            ...
            return masks_sam, masks_hq, edge_pred, inter_masks_hq, inter_edge_pred, masks_dense, mask_feats
    ...
    def predict_masks(
        ...
        inter_mask_hq_token = self.hf_mlp(inter_mask_tokens_out[:, -2, :] + inter_mask_tokens_out[:, -1, :])
        ...
        return masks, inter_masks, dense_, iou_pred, upscaled_edge_embedding
...
def main(net, train_datasets, valid_datasets, args):
    ...
    if not args.eval:
        if args.restore_model:
            ...
            if torch.cuda.is_available():
                net_without_ddp.load_state_dict(torch.load(args.restore_model, map_location={'cuda:0': 'cuda:4'}))
            else:
        ...
        hq_decoder = torch.load(args.output + 'epoch_11.pth')
        ...
        exit()
        evaluate(args, net, sam, valid_dataloaders, False , args.visualize)
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    # test
    test_stats = evaluate(args, net, sam, valid_dataloaders, save_preds=False)
...
def evaluate(args, net, sam, valid_dataloaders, save_preds=False, visualize=False):
    ...
    print("Validating...")
    test_stats = {}
    dataset_name = ['CHAMELEON','CAMO', 'COD10K', 'NC4K', 'DIS']
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            masks_sam, masks_hq, edge_pred, inter_masks_hq, inter_edge, masks_dense, mask_feats = net(
                image_embeddings=encoder_embedding,
                image_pe=image_pe,
                sparse_prompt_embeddings=sparse_embeddings,
                dense_prompt_embeddings=dense_embeddings,
            ...
            masks_hq = masks_hq    # mask coupled 
...
if __name__ == "__main__":
    ...
    # valid set
    dataset_dislow_val = {"name": "DIS5K-VD",
    ...
    # guo
    train_datasets=[dataset_cod, dataset_dis_low, dataset_dis, dataset_thin, dataset_fss, dataset_duts, dataset_duts_te, dataset_ecssd, dataset_msra]
