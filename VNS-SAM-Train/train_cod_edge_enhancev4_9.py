...
class MaskDecoderHQ(MaskDecoder):
    ...
    def forward(
        ...
        label: torch.Tensor, # label  # b 1 1024 1024
        ...
        label = F.interpolate(label, size=image_embeddings.shape[2:], mode='bilinear', align_corners=False).squeeze(1)
        ...
        for i_batch in range(batch_len):
            ...
            foreground_feat = image_embeddings[i_batch].permute(1,2,0)[label[i_batch]>0]
            if len(foreground_feat) == 0:
                foreground_feat = torch.zeros((1,256), device=foreground_feat.device)
            # foreground_feat = foreground_feat.mean(0).unsqueeze(0)
            background_feat = image_embeddings[i_batch].permute(1,2,0)[label[i_batch] == 0]
            
            
            foreground_feat = foreground_feat.mean(0).unsqueeze(0)  # 1 256
            background_feat = background_feat.mean(0).unsqueeze(0)  # 1 256
            ...
            foreground_feats.append(foreground_feat)
            background_feats.append(background_feat)
            
            
        masks = torch.cat(masks,0)
        iou_preds = torch.cat(iou_preds,0)
        mask_feats = torch.cat(mask_feats, 0)
        foreground_feats = torch.cat(foreground_feats, 0)
        background_feats = torch.cat(background_feats, 0)
        ...
        if hq_token_only:
            return masks_hq, edge_pred, (foreground_feats, background_feats)
        else:
            return masks_sam, masks_hq, edge_pred, mask_feats
...
def InfoNCEloss(q, k):
    ...
    num_sample = q.shape[1]
    feature_dim = q.shape[2]
    q = q.view(batch_size * num_sample, feature_dim)
    k = k.view(batch_size * num_sample, feature_dim)
    sim = F.cosine_similarity(q, k, dim=1)
    print(sim)
    print(sim.shape)

    labels = torch.arange(batch_size * num_sample, dtype=torch.int64).to(sim.device)
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            masks_hq, edge_pred, (fore_feats, back_feats) = net(
            ...
            loss_ctrs = args.weight * loss_contrast(fore_feats, back_feats)
            
            loss = (loss_mask + loss_dice) + loss_edge +  loss_ctrs
            
            loss_dict = {"loss_mask": loss_mask, "loss_dice":loss_dice, "loss_edge": loss_edge, "loss_contrast": loss_ctrs}
...
def loss_contrast(f_feats, b_feats):

    distance = F.pairwise_distance(f_feats, b_feats)

    loss = torch.pow(torch.clamp(5 - distance, min=0.0), 2).mean()
    ...
    #     f_feat = f_feats[i]
    #     b_feat = b_feats[i]
    #     distance = F.pairwise_distance(f_feat, b_feat)
    #     print(distance)
...
def evaluate(args, net, sam, valid_dataloaders, save_preds=False, visualize=False):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            masks_sam, masks_hq, edge_pred, mask_feats = net(
                image_embeddings=encoder_embedding,
                image_pe=image_pe,
                sparse_prompt_embeddings=sparse_embeddings,
                dense_prompt_embeddings=dense_embeddings,
