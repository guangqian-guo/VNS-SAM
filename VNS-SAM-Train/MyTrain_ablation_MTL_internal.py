...
class MaskDecoderEnhance(MaskDecoder):
    ...
    def forward(
        ...
        for i_batch in range(batch_len):
            mask, inter_mask, iou_pred, mask_feat, attn_map = self.predict_masks(
        ...
        if hq_token_only:
            ...
            return masks_sam, masks_hq, edge_pred, inter_masks_hq, inter_edge_pred,  mask_feats, attn_maps
    ...
    def predict_masks(
        ...
        upscaled_embedding_ours = (self.embedding_maskfeature(upscaled_embedding_sam)) # b 32 256 256

        upscaled_inter_embedding_ours = self.embedding_maskfeature(self.output_upscaling(internal_src))
        
        # vis ------------------------------
        # heatmap(upscaled_embedding_ours, None)
        #-----------------------------------
        
        upscaled_edge_embedding = self.embedding_edgefeature(upscaled_embedding_sam)
        upscaled_inter_edge_embedding = self.embedding_edgefeature(self.output_upscaling(internal_src))
        ...
        return masks, inter_masks, iou_pred, edge_pred, attn_map
...
def evaluate(args, net, sam, valid_dataloaders, val_set_name, save_preds=False, visualize=False):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            masks_sam, masks_hq, edge_pred, inter_masks_hq, inter_edge, mask_feats, attn_maps = net(
                image_embeddings=encoder_embedding,
                image_pe=image_pe,
                sparse_prompt_embeddings=sparse_embeddings,
                dense_prompt_embeddings=dense_embeddings,
