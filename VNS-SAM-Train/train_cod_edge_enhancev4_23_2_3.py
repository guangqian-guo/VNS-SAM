...
class MaskDecoderHQ(MaskDecoder):
    def __init__(self, model_type):
        ...
        self.dense_pred_layer = nn.Sequential(
            nn.Linear(transformer_dim, transformer_dim // 8),
            nn.Linear(transformer_dim // 8, transformer_dim),
        )
        self.dense_pred_head = nn.Sequential(
            ...
            nn.Conv2d(transformer_dim, transformer_dim, kernel_size=3, stride=1, padding=1),
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
            nn.Conv2d(transformer_dim, 1, kernel_size=3, stride=1, padding=1)
        )
    ...
    def predict_masks(
        ...
        dense_pred = self.dense_pred_layer(dense_pred)
...
def evaluate(args, net, sam, valid_dataloaders, save_preds=False, visualize=False):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            masks_hq = masks_hq + masks_sam + inter_masks_hq + masks_dense   # mask coupled 
