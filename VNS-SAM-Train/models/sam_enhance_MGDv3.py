...
class MaskDecoderMGDv3(MaskDecoder):
    def __init__(self, model_type):
        ...
        self.hf_mlp_2 = MLP(transformer_dim//8, transformer_dim//8, transformer_dim // 8, 1)
        
        self.edge_mlp = (MLP(transformer_dim, transformer_dim, transformer_dim // 8, 3))
        self.edge_mlp_2 = (MLP(transformer_dim//8, transformer_dim//8, transformer_dim // 8, 1))
    ...
    def predict_masks(
        ...
        #-----------------------------------
        upscaled_edge_embedding = self.embedding_edgefeature(upscaled_embedding_sam) + edge_feat
        
        
        upscaled_inter_edge_embedding = self.embedding_edgefeature(self.output_upscaling(internal_src)) + edge_feat
        ...
        # fusion_token = mask_tokens_out[:, -2, :] + mask_tokens_out[:, -1, :]
        
        edge_token = self.edge_mlp(mask_tokens_out[:, -1, :])
        mask_hq_token = self.hf_mlp_2(self.hf_mlp(mask_tokens_out[:, -2, :]) + edge_token)  # b 32
        edge_token = self.edge_mlp_2(edge_token + mask_hq_token) 
        
        
        inter_edge_token = self.edge_mlp(inter_mask_tokens_out[:, -1, :])
        inter_mask_hq_token = self.hf_mlp_2(self.hf_mlp(inter_mask_tokens_out[:, -2, :]) + inter_edge_token)
        inter_edge_token = self.edge_mlp_2(inter_edge_token + inter_mask_hq_token)
