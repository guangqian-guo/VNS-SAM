...
class TokenInter(nn.Module):
    ...
    def forward(self, mask_token, edge_token):
        ...
        fusion_token = self.f1(fusion_token)

        # mask toekn
        mask_token_o = self.token_attn(fusion_token, mask_token, mask_token)

        # edge token
        edge_token_o = self.token_attn(fusion_token, edge_token, edge_token)

        return mask_token_o, edge_token_o        
