...
# from train.segment_anything_training.modeling import transformer
...
class MaskDecoderEnhanceS(MaskDecoder):
    def __init__(self, model_type):
        ...
        # print(self.transformer.layers.0.cross_attn_token_to_image.v_proj.weight)
        ...
        self._LoRA_decoder(r=4)
        
        
        for n, p in self.named_parameters():
            # lora and mlp and upscale
            if "iou" not in n:
                p.requires_grad = True
            if ("transformer" in n and ("w_a" not in n and "w_b" not in n and "internal" not in n)):
                p.requires_grad = False
            
            # mlp and upscale
            # if "w_a" in n or "w_b" in n or "internal"  in n:
            #     p.requires_grad = True
            

    def _LoRA_decoder(self, r):
        self.self_attn_As = []
        self.self_attn_Bs = []
        ...
        block = self.transformer.final_attn_token_to_image
    ...
    def forward(
        ...
        if hq_token_only:
            return masks_sam, masks_hq, edge_pred, inter_masks_hq, inter_edge_pred, masks_dense
        else:
