...
parser.add_argument("--mask-dir", help='such as data/coco/resize/annotations/instances_val2017_100x167.json', default='data/COCO/annotations/instances_train2017.json')
parser.add_argument("--pred-dir", help='such as exp/latest_result.json',default=None)
args = parser.parse_args()

# data_root = "./test_data"
# mask_root = os.path.join(data_root, "masks")
# pred_root = os.path.join(data_root, "preds")
mask_dataset_list = sorted(os.listdir(args.mask_dir))
pred_dataset_list = sorted(os.listdir(args.pred_dir))
# mask_name_list = sorted(os.listdir(args.mask_dir))
for i, dataset in enumerate(mask_dataset_list):
    print(f"================[{i}] Processing {dataset}================")
    ...
    for j, mask_name in enumerate(pred_name_list):
