...
class MaskDecoderHQ(MaskDecoder):
    def __init__(self, model_type):
        ...
        self.num_hf_tokens = 3
        self.num_edge_tokens = 1
        self.hf_token = nn.Embedding(self.num_hf_tokens + self.num_edge_tokens, transformer_dim)  # 三个mask token， 一个edge token
        # self.hf_mlp = MLP(transformer_dim, transformer_dim, transformer_dim // 8, 3)
        self.hf_mlp = nn.ModuleList(
                            [
                                MLP(transformer_dim, transformer_dim, transformer_dim // 8, 3)
                                for i in range(self.num_hf_tokens)
                            ]
                        )
        self.edge_mlp = MLP(transformer_dim, transformer_dim, transformer_dim // 8, 3)

        self.num_mask_tokens = self.num_mask_tokens + self.num_hf_tokens + self.num_edge_tokens
    ...
    def forward(
        ...
        for i_batch in range(batch_len):
            mask, iou_pred, mask_feat, mask_hq_token = self.predict_masks(
            ...
            mask_hq_tokens.append(mask_hq_token)
            
        masks = torch.cat(masks,0)
        iou_preds = torch.cat(iou_preds,0)
        mask_feats = torch.cat(mask_feats, 0)
        mask_hq_tokens = torch.cat(mask_hq_tokens)
        ...
        masks_hq = masks[:,slice(self.num_mask_tokens-self.num_hf_tokens-self.num_edge_tokens, self.num_mask_tokens-self.num_edge_tokens), :, :]
        masks_hq = torch.mean(masks_hq, dim=1, keepdim=True)

        edge_pred = torch.sigmoid(masks[:,slice(self.num_mask_tokens-self.num_edge_tokens, self.num_mask_tokens), :, :])
        
        if hq_token_only:
            return masks_hq, edge_pred, mask_hq_tokens
        else:
    ...
    def predict_masks(
        ...
        for i in range(self.num_mask_tokens):
            if i < 4:
                ...
                mask_hq_tokens.append(self.hf_mlp[i-4](mask_tokens_out[:, i, :]))
            # elif i < 5:
        ...
        mask_hq_token = torch.stack(mask_hq_tokens, dim=1)
        ...
        return masks, iou_pred, upscaled_edge_embedding, mask_hq_token
...
import itertools
def loss_tokens(tokens):
    cosine_sim_list = []
    for b in range(len(tokens)):
        cosine_similarity_sum = 0.0
        for pair in itertools.combinations(tokens[b], 2):
            sim = torch.nn.functional.cosine_similarity(pair[0], pair[1], dim=0)
            cosine_similarity_sum += sim
        cosine_sim_list.append(cosine_similarity_sum)
    return torch.tensor(cosine_sim_list, device=tokens.device).mean()
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            masks_hq, edge_pred, masks_hq_tokens = net(
            ...
            loss_token = loss_tokens(masks_hq_tokens)

            loss = (loss_mask + loss_dice) + loss_edge + loss_token
            
            loss_dict = {"loss_mask": loss_mask, "loss_dice":loss_dice, "loss_edge": loss_edge, "loss_token": loss_token}
...
def evaluate(args, net, sam, valid_dataloaders, save_preds=False, visualize=False):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            masks_hq = masks_hq + masks_sam  # mask coupled 
