...
class SAMAggregatorNeck(nn.Module):
    ...
    def forward(self, inputs):
        ...
        edge_feat = self.up_sample_layers[1](edge_feat)
...
class MaskDecoderHQ(MaskDecoder):
    ...
    def predict_masks(
        ...
        upscaled_embedding_ours = (self.embedding_maskfeature[-1](upscaled_embedding_sam) + hq_feature + 0.5 * edge_feat) # b 32 256 256

        upscaled_inter_embedding_ours = self.embedding_maskfeature(self.output_upscaling(internal_src)) + hq_feature + 0.5 * edge_feat
...
def evaluate(args, net, sam, valid_dataloaders, save_preds=False, visualize=False):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            # masks_hq = masks_sam# mask coupled 
