...
class MaskDecoderHQ(MaskDecoder):
    def __init__(self, model_type):
        ...
        self.hf_token = nn.Embedding(3, transformer_dim)  # 一个foreground token， background token, 一个edge token
        self.hf_mlp_f = MLP(transformer_dim, transformer_dim, transformer_dim // 8, 3)
        self.hf_mlp_b = MLP(transformer_dim, transformer_dim, transformer_dim // 8, 3)
        self.edge_mlp = MLP(transformer_dim, transformer_dim, transformer_dim // 8, 3)

        self.num_mask_tokens = self.num_mask_tokens + 3
        ...
        self.embedding_fmaskfeature = nn.Sequential(
                                        nn.Conv2d(transformer_dim // 8, transformer_dim // 4, 3, 1, 1), 
                                        LayerNorm2d(transformer_dim // 4),
                                        nn.GELU(),
                                        nn.Conv2d(transformer_dim // 4, transformer_dim // 8, 3, 1, 1))
        # self.embedding_bmaskfeature = nn.Sequential(
        #                                 nn.Conv2d(transformer_dim // 8, transformer_dim // 4, 3, 1, 1), 
        #                                 LayerNorm2d(transformer_dim // 4),
        #                                 nn.GELU(),
        #                                 nn.Conv2d(transformer_dim // 4, transformer_dim // 8, 3, 1, 1))
    ...
    def forward(
        ...
        for i_batch in range(batch_len):
            mask, iou_pred, mask_feat, (f_token, b_token) = self.predict_masks(
        ...
        f_tokens = torch.cat(f_tokens, 0)
        b_tokens = torch.cat(b_tokens, 0)
        ...
        f_masks_hq = masks[:,slice(self.num_mask_tokens-3, self.num_mask_tokens-2), :, :]
        b_masks_hq = masks[:,slice(self.num_mask_tokens-2, self.num_mask_tokens-1), :, :]
        edge_pred = torch.sigmoid(masks[:,slice(self.num_mask_tokens-1, self.num_mask_tokens), :, :])
        
        if hq_token_only:
            return f_masks_hq, b_masks_hq, edge_pred, (f_tokens, b_tokens)
        else:
            return masks_sam, f_masks_hq, b_masks_hq, edge_pred, mask_feats
    ...
    def predict_masks(
        ...
        upscaled_embedding_ours_f = (self.embedding_fmaskfeature(upscaled_embedding_sam) + hq_feature) # b 32 256 256
        # upscaled_embedding_ours_b = self.embedding_bmaskfeature(upscaled_embedding_sam) + hq_feature
        ...
        f_mask_hq_token = self.hf_mlp_f(mask_tokens_out[:, -3, :] + mask_tokens_out[:, -2, :])  # b 32
        b_mask_hq_token = self.hf_mlp_b(mask_tokens_out[:, -2, :] + mask_tokens_out[:, -1, :])  # b 32
        ...
        f_masks_ours = (f_mask_hq_token @ upscaled_embedding_ours_f.view(b, c, h * w)).view(b, -1, h, w)  # 和mask token做点乘，作为输出的mask。
        b_masks_ours = (b_mask_hq_token @ upscaled_embedding_ours_f.view(b, c, h * w)).view(b, -1, h, w)

        edge_pred = (edge_token @ upscaled_edge_embedding.view(b, c, h * w)).view(b, -1, h, w)
        
        masks = torch.cat([masks_sam, f_masks_ours, b_masks_ours, edge_pred],dim=1)

        iou_pred = self.iou_prediction_head(iou_token_out)
        
        return masks, iou_pred, upscaled_edge_embedding, (f_mask_hq_token, b_mask_hq_token)
...
def loss_tokens(f_tokens, b_tokens): # 4 ,32
    loss = nn.L1Loss()(f_tokens, -1*b_tokens).mean()
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            f_masks_hq, b_masks_hq, edge_pred, (f_tokens, b_tokens) = net(
            ...
            loss_mask_f, loss_dice_f = loss_masks(f_masks_hq, labels/255.0, len(f_masks_hq))
            loss_mask_b, loss_dice_b = loss_masks(b_masks_hq, 1-(labels/255.0), len(b_masks_hq))

            loss_edge = dice_loss_edge(edge_pred, edges)   # TODO !!!
            
            loss_token = loss_tokens(f_tokens, b_tokens)

            loss = (loss_mask_f + loss_dice_f) +  loss_edge + loss_mask_b + loss_dice_b + loss_token
            
            loss_dict = {"loss_mask_foreground": loss_mask_f, "loss_dice_foregrond":loss_dice_f, "loss_mask_background": loss_mask_b, "loss_dice_background":loss_dice_b, "loss_edge": loss_edge, "loss_token": loss_token}
...
def evaluate(args, net, sam, valid_dataloaders, save_preds=False, visualize=False):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            masks_sam, f_masks_hq, b_masks_hq, edge_pred, mask_feats = net(
                image_embeddings=encoder_embedding,
                image_pe=image_pe,
                sparse_prompt_embeddings=sparse_embeddings,
                dense_prompt_embeddings=dense_embeddings,
            ...
            masks_hq = (f_masks_hq - b_masks_hq) + masks_sam  # mask coupled 
            # masks_hq = (f_masks_hq - b_masks_hq)/2.0 # mask coupled
