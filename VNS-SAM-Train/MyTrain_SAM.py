...
def visualize_attention_map_opencv(attention_weights, img, name=None):
    ...
    if img is not None:
        x_show = cv2.addWeighted(img, 0.6, heatmap, 0.4, 0)
        # x_show = img
    if name is not None:
        cv2.imwrite(name, x_show)
...
def evaluate(args, net, sam, valid_dataloaders, val_set_name, save_preds=False, visualize=False):
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            masks_sam = batched_output[0]['low_res_logits']
            ...
            res = F.upsample(masks_sam, size=tuple(labels_ori.shape[2:]), mode='bilinear', align_corners=False)
            ...
            # show_saliency_map(mask_feats[:, 0, :, :], inputs_val.squeeze(0), os.path.join(save_dir, img_name))
            # vis attntion map
            # vis_attn_map = attn_maps[0, -2, :, 1].squeeze().reshape(64,64)
            ...
            if visualize:
                ...
                os.makedirs(args.output, exist_ok=True)
                os.makedirs(save_dir, exist_ok=True)
                masks_hq_vis = (F.interpolate(masks_sam.detach(), (1024, 1024), mode="bilinear", align_corners=False) > 0).cpu()
                for ii in range(len(imgs)):
                    base = data_val['imidx'][ii].item()
                    # print('base:', base)
