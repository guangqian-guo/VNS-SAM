...
class MaskDecoderHQ(MaskDecoder):
    def __init__(self, model_type):
        ...
        self.hf_mlp = MLP(transformer_dim, transformer_dim, transformer_dim, 3)
    ...
    def predict_masks(
        ...
        mask_hq_token = self.hf_mlp(mask_tokens_out[:, -2, :] + mask_tokens_out[:, -1, :])  # b 256
        ...
        b_, c_, h_, w_ = upscaled_embedding_ours.shape
        masks_ours = (mask_hq_token @ upscaled_embedding_ours.view(b_, c_, h_ * w_)).view(b_, -1, h_, w_)  # 和mask token做点乘，作为输出的mask。
