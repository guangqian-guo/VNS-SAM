...
class SAMAggregatorNeck(nn.Module):
    ...
    def forward(self, inputs):
        inputs = [einops.rearrange(inputs[idx], 'b h w c -> b c h w') for idx in range(len(self.in_channels))]
        x = self.down_sample_layer(inputs[-1])
        # edge pred
...
class MaskDecoderEnhance(MaskDecoder):
    ...
    def forward(
        ...
        for i_batch in range(batch_len):
            mask, inter_mask, dense_pred, iou_pred, mask_feat, attn_map = self.predict_masks(
        ...
        if hq_token_only:
            ...
            return masks_sam, masks_hq, edge_pred, inter_masks_hq, inter_edge_pred, masks_dense, mask_feats, attn_maps
    ...
    def predict_masks(
        ...
        return masks, inter_masks, dense_, iou_pred, edge_pred, attn_map
...
if __name__ == "__main__":
    ...
    net = MaskDecoderEnhance(args.model_type)
