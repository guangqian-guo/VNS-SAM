...
from utils.VNS_score2 import compute_vns_score
...
def generate_benchmark_table(results):
    ...
    for k in range(len(results)):
        ...
        iou = '.' + str('%.3f'%result['iou']).split('.')[-1]
        w_F = '.' + str('%.3f'%result['wFmeasure']).split('.')[-1]
        mean_E_m = '.' + str('%.3f'%result['meanEm']).split('.')[-1]
        biou = '.' + str('%.3f'%result['biou']).split('.')[-1]
        res_latex = '& {}   & {}   & {}   & {} \n'.format(iou, biou, mean_E_m, w_F)
...
def show_anns(masks, input_point, input_box, input_label, filename, image, ious, boundary_ious):
    ...
    for i, (mask) in enumerate(masks):
        ...
        score_text = f'IoU: {ious:.3f} BIoU: {boundary_ious:.3f}'
        plt.gca().text(20, 35, score_text,
                      color='green', fontsize=20, backgroundcolor='white',
...
def get_args_parser():
    ...
    parser.add_argument('--results_fusion', action='store_true',
                        help='fuse the results of hq and sam.')
    parser.add_argument("--k", type=int, default=10,
                        help="the number of the point prompt")
...
def main(net, train_datasets, valid_datasets, args):
    ...
    if not args.eval:
        ...
        evaluate(args, net, sam, valid_dataloaders, val_set_name, False, args.visualize)
...
def train(args, net, optimizer, train_dataloaders, valid_dataloaders, val_set_name, lr_scheduler):
    ...
    for epoch in range(epoch_start, epoch_num): 
        ...
        for data in metric_logger.log_every(train_dataloaders,100):
            ...
            masks_hq, edge_pred, inter_masks_hq, inter_edge_pred, fusion_mask = net(
            ...
            # loss_mask2, loss_dice2 = loss_masks(inter_masks_hq, labels/255.0, len(inter_masks_hq))
            loss_mask2 = structure_loss(inter_masks_hq, labels/255.0)
            
            if inter_edge_pred is not None:
                loss_edge2 = dice_loss_edge(inter_edge_pred, edges)
            else:
                loss_edge2 = torch.tensor(0,device='cuda')

            loss2 = loss_mask2 + loss_edge2
            
            # loss_mask_dense, loss_dice_dense = loss_masks(masks_dense, labels/255.0, len(masks_dense))
            # loss_dense = loss_mask_dense + loss_dice_dense
            
            loss_fusion_mask = structure_loss(fusion_mask, labels/255.0 )
            
            # loss = 0.25 * loss1 + 0.25 * loss2 + 0.5 * loss_dense + loss_fusion_mask 
            loss = (loss1 + loss2) + loss_fusion_mask 
            
            # loss_dict = {"loss_mask": loss_mask, "loss_dice":loss_dice, "loss_edge": loss_edge}
            if edge_pred is not None:
                loss_dict = {"loss_edge1": loss_edge1, 
                            "loss_mask2": loss_mask2,  "loss_edge2": loss_edge2,
                            "loss_fusion_mask": loss_fusion_mask}
            else:
                loss_dict = {"loss_mask2": loss_mask2, 
                            "loss_fusion_mask": loss_fusion_mask}
...
def evaluate(args, net, sam, valid_dataloaders, val_set_name, save_preds=False, visualize=False):
    ...
    mean_wfm = mean_seen_wfm = mean_unseen_wfm = 0
    mean_em = mean_seen_em = mean_unseen_em = 0
    mean_iou = mean_seen_iou = mean_unseen_iou = 0
    mean_biou = mean_seen_biou = mean_unseen_biou = 0
    mean_dice = mean_seen_dice = mean_unseen_dice = 0
    seen_set_name = ['CAMO', 'COD10K', 'Kvasir', 'CVC-ClinicDB', 'DIS5K-VD-low', 'ThinObject5k-TE-low']
    unseen_set_name = ['NC4K', 'CVC-ColonDB', 'ETIS', 'cds2k']    

    scores = []  # List to store VNS scores and IoU
    ...
    for k in range(len(valid_dataloaders)):
        ...
        for data_val in metric_logger.log_every(valid_dataloader, 100):
            ...
            labels_noise_box = misc.box_noise(labels_box, box_noise_scale=0.1)
            labels_points = misc.masks_sample_points(labels_val[:,0,:,:], k=args.k)  # k is the number of point
            ...
            for b_i in range(len(imgs)):
                ...
                if input_type == 'box':
                    dict_input['boxes'] = labels_box[b_i:b_i+1]
                elif input_type == 'noise_box':
                    dict_input['boxes'] = labels_noise_box[b_i: b_i+1]
                elif input_type == 'point':
                    point_coords = labels_points[b_i:b_i+1]
                    dict_input['point_coords'] = point_coords
                    dict_input['point_labels'] = torch.ones(point_coords.shape[1], device=point_coords.device)[None,:]

                    # point_coords = labels_pos_neg_points[b_i:b_i+1]
                    # dict_input['point_coords'] = point_coords
                    # pos_point_labels = torch.ones(point_coords.shape[1] // 2, device=point_coords.device)

                    # neg_point_labels = torch.zeros(point_coords.shape[1] // 2, device=point_coords.device)
                    # dict_input['point_labels'] = torch.cat([pos_point_labels, neg_point_labels], dim=0)[None, :]
                elif input_type == 'noise_mask':
                    dict_input['mask_inputs'] = labels_noisemask[b_i:b_i+1]
                else:
                    raise NotImplementedError
            ...
            masks_sam, masks_hq, edge_pred, inter_masks_hq, inter_edge, fusion_pred, mask_feats, attn_maps = net(
                image_embeddings=encoder_embedding,
                image_pe=image_pe,
                sparse_prompt_embeddings=sparse_embeddings,
                dense_prompt_embeddings=dense_embeddings,
            ...
            if args.results_fusion:
                masks_hq = fusion_pred + masks_sam # mask coupled 
            else:
                masks_hq = fusion_pred 
            # vis heatmap
            # os.makedirs(args.output, exist_ok=True)
            ...
            dice = compute_dice_similarity_coefficient(masks_hq, labels_ori)
            
            # # Compute VNS score
            # mask_path = ori_gt_path[0]
            # image_path = mask_path.replace('masks' if "masks" in mask_path else "GT", 'images' if "masks" in mask_path else "Imgs").replace('.png', '.jpg')
            # _, _, _, vns_score = compute_vns_score(image_path, mask_path)
            # # # Save VNS score and IoU
            # scores.append({'image': img_name, 'vns_score': vns_score, 'iou': iou.item(), 'biou': boundary_iou.item()})
            ...
            if visualize:
                ...
                for ii in range(len(imgs)):
                    ...
                    show_iou = torch.tensor(iou)
                    show_boundary_iou = torch.tensor(boundary_iou)
                    # show_anns(masks_hq_vis[ii], None, labels_box[ii].cpu(), None, save_base , imgs_ii, show_iou, show_boundary_iou)   
                    show_anns(masks_hq_vis[ii], labels_points[ii].cpu(), None, np.ones(labels_points.shape[1]), save_base , imgs_ii, show_iou, show_boundary_iou)    
            ...
            loss_dict = {"val_iou_"+str(k): iou, "val_boundary_iou_"+str(k): boundary_iou, "Dice"+str(k): dice}
            loss_dict_reduced = misc.reduce_dict(loss_dict)
        ...
        all_gather_object(all_outputs, wfm)
        averaged_wfm_all_process = sum(all_outputs) / len(all_outputs)
        all_gather_object(all_outputs, em["curve"].mean())
        averaged_em_all_process = sum(all_outputs) / len(all_outputs)
        
        resstat = {k: meter.global_avg for k, meter in metric_logger.meters.items() if meter.count > 0}
        test_stats.update(resstat)

        for key in resstat.keys():
            if 'boundary' in key:
                mbiou = resstat[key]
            elif 'iou' in key:
                miou = resstat[key]
        ...
        curr_results = {
            "meanEm": averaged_em_all_process,
            "wFmeasure": averaged_wfm_all_process,
            "iou": miou,
            "biou": mbiou,
            "dice": dice
        }
        ...
        mean_wfm += averaged_wfm_all_process
        mean_em += averaged_em_all_process
        mean_iou += miou
        mean_biou += mbiou
        mean_dice += dice
        
        if val_set_name[k] in seen_set_name:
            mean_seen_iou += miou
            mean_seen_biou += mbiou
            mean_seen_wfm += averaged_wfm_all_process
            mean_seen_em += averaged_em_all_process
                
        elif val_set_name[k] in unseen_set_name:
            mean_unseen_iou += miou
            mean_unseen_biou += mbiou
            mean_unseen_wfm += averaged_wfm_all_process
            mean_unseen_em += averaged_em_all_process  
    ...
    # all
    mean_wfm = mean_wfm / len(valid_dataloaders)
    mean_em = mean_em / len(valid_dataloaders)
    mean_iou = mean_iou / len(valid_dataloaders)
    mean_biou = mean_biou / len(valid_dataloaders)
    mean_dice = mean_dice / len(valid_dataloaders)
    
    # seen
    mean_seen_wfm = mean_seen_wfm / len(seen_set_name)
    mean_seen_em = mean_seen_em / len(seen_set_name)
    mean_seen_iou = mean_seen_iou / len(seen_set_name)
    mean_seen_biou = mean_seen_biou / len(seen_set_name)
    
    # unseen
    mean_unseen_wfm = mean_unseen_wfm / len(unseen_set_name)
    mean_unseen_em = mean_unseen_em / len(unseen_set_name)
    mean_unseen_iou = mean_unseen_iou / len(unseen_set_name)
    mean_unseen_biou = mean_unseen_biou / len(unseen_set_name) 
    
    print('%.3f'%mean_iou, '%.3f'%mean_biou, '%.3f'%mean_em, '%.3f'%mean_wfm )
    print('%.3f'%mean_seen_iou, '%.3f'%mean_seen_biou, '%.3f'%mean_seen_em, '%.3f'%mean_seen_wfm)
    print('%.3f'%mean_unseen_iou, '%.3f'%mean_unseen_biou, '%.3f'%mean_unseen_em, '%.3f'%mean_unseen_wfm)
    
    latex = generate_benchmark_table(results)
    if dist.get_rank() == 0:
        logging.info('&{}  &{}  &{}   &{}'.format(mean_iou, mean_biou, mean_em, mean_wfm))
        logging.info(latex)
